...
export default function Home() {
  return (
    <main className="min-h-screen relative">
      ...
      <section id="services" className="py-24 relative z-10">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <h2 className="font-display text-3xl md:text-5xl font-bold text-white mb-4">What We Deliver</h2>
            <p className="text-slate-400 max-w-2xl mx-auto">Comprehensive digital solutions combining cutting-edge engineering with robust cybersecurity.</p>
          </div>
      ...
      {/* Projects Preview */}
      <section id="projects" className="py-24 relative z-10 bg-slate-900/30">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div>
              <h2 className="font-display text-3xl md:text-5xl font-bold text-white mb-4">Innovation in Action</h2>
              <p className="text-slate-400 max-w-xl">A glimpse into our portfolio of secured, high-performing applications.</p>
            </div>
            <Link href="/projects" className="btn-outline px-6 py-3 rounded-full text-white text-sm font-medium w-max shrink-0">
              View All Projects
            </Link>
          </div>
      ...
      <section id="contact" className="py-32 relative z-10 flex items-center justify-center">
        <div className="max-w-4xl mx-auto px-6 w-full relative">
          ...
          <div className="glass-card p-12 md:p-16 rounded-3xl text-center border-t border-white/20">
            <h2 className="font-display text-4xl md:text-5xl font-bold text-white mb-6">Let&apos;s Collaborate</h2>
            <p className="text-lg text-slate-300 max-w-2xl mx-auto mb-10">Ready to transform your ideas into reality? Partner with AdhritVerse to architect a secure and innovative digital future for your business.</p>
            
            <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
              <a href="mailto:contact@adhritverse.in" className="btn-primary px-10 py-4 rounded-full text-white font-medium w-full sm:w-auto text-lg whitespace-nowrap">
                Request a Quote
              </a>
              <a href="tel:+918462802086" className="btn-outline px-10 py-4 rounded-full text-white font-medium w-full sm:w-auto text-lg whitespace-nowrap flex items-center justify-center gap-2">
                <i className="fas fa-phone-alt text-sm"></i> Contact Us
              </a>
            </div>
          </div>
        </div>
...
function ServiceCard({ icon, title, desc, color, bg }: { icon: string; title: string; desc: string; color: string; bg: string }) {
  return (
    <div className="glass-card p-8 rounded-2xl group cursor-pointer">
      <div className={`w-14 h-14 rounded-xl ${bg} flex items-center justify-center ${color} text-2xl mb-6 group-hover:scale-110 transition-transform`}>
        <i className={icon}></i>
      </div>
      <h3 className="text-xl font-bold text-white mb-3">{title}</h3>
      <p className="text-slate-400 text-sm mb-6 line-clamp-3">{desc}</p>
      <Link href="/services" className={`${color} hover:text-white font-medium text-sm flex items-center gap-1 transition-colors`}>
    ...
    <a href={href} target="_blank" rel="noopener noreferrer" className="glass-card rounded-2xl overflow-hidden group cursor-pointer transition-all duration-300 hover:-translate-y-2">
      <div className="h-48 bg-slate-800 relative overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute bottom-4 left-4 flex gap-2">
          {tags.map((tag) => (
            <span key={tag} className="px-2 py-1 text-xs font-medium rounded bg-primary/80 text-white backdrop-blur-sm">{tag}</span>
          ))}
        </div>
      </div>
      <div className="p-6">
